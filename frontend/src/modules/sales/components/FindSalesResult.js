...
const FindSalesResult = () => {
    ...
    const sales = [
        ...
        },
        {
            ...
            client: "Paco Perez",
            agent: "Samuel Lopez",
            travels: ["Chicago-BuenosAires", "Chicago-BuenosDias", "Chicago-BuenasNoches"],
            acommodations: ["hesperia"],
            transports: ["catamaran"],
            activities: ["surf"],
            price: 1000.95,
                        ...
                        <TableBody>
                            {sales.map((sale) =>
                                <TableRow key={sale.id}>
                                    ...
                                    <TableCell align="center">
                                        <Box
                                            onMouseEnter={event => handlePopoverOpen(event, "travels" + sale.id)}
                                    ...
                                    <TableCell align="center">
                                        <Box
                                            onMouseEnter={event => handlePopoverOpen(event, "acommodations" + sale.id)}
                                            onMouseLeave={handlePopoverClose}
                                        >
                                            <Badge badgeContent={sale.acommodations.length} color="primary">
                                                <HotelIcon />
                                            </Badge>
                                        </Box>
                                        <MyPopover id={"acommodations" + sale.id} items={sale.acommodations} />
                                    </TableCell>
                                    <TableCell align="center">
                                        <Box
                                            onMouseEnter={event => handlePopoverOpen(event, "transports" + sale.id)}
                                            onMouseLeave={handlePopoverClose}
                                        >
                                            <Badge badgeContent={sale.transports.length} color="primary">
                                                <CommuteIcon />
                                            </Badge>
                                        </Box>
                                        <MyPopover id={"transports" + sale.id} items={sale.transports} />
                                    </TableCell>
                                    <TableCell align="center">
                                        <Box
                                            onMouseEnter={event => handlePopoverOpen(event, "activities" + sale.id)}
                                            onMouseLeave={handlePopoverClose}
                                        >
                                            <Badge badgeContent={sale.activities.length} color="primary">
                                                <SurfingIcon />
                                            </Badge>
                                        </Box>
                                        <MyPopover id={"activities" + sale.id} items={sale.activities} />
                                    </TableCell>
                                    <TableCell align="center">
                                        <Typography variant="h6">
                                            {sale.price}€
